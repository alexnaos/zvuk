...
void setup()
{
    ...
    // Инициализируем Аппаратный Watchdog на 8 секунд
    ...
    // Ждем роутер не более 10 секунд. Если он не готов — идем дальше
    ...
    // Сбрасываем WDT перед запуском сетевых служб, так как у нас уже есть подключение
    esp_task_wdt_reset();

    // Настраиваем получение времени через интернет (NTP) только при наличии связи
    if (WiFi.status() == WL_CONNECTED)
    {
        configTime(3600 * 3, 0, "pool.ntp.org", "ru.pool.ntp.org");
        Serial.println("[Время]: Синхронизация NTP запущена.");
    }

    // Кормим ватчдог, так как функция configTime() могла сделать короткую задержку на DNS-запрос
    esp_task_wdt_reset();

    // Инициализируем тяжелые сетевые библиотеки
    initWebServer();
    esp_task_wdt_reset(); // Сервер запущен, сбрасываем таймер

    initMQTT();
    esp_task_wdt_reset(); // MQTT запущен, сбрасываем таймер перед уходом в loop()
...
void loop()
{
    ...
    // Каждую секунду обновляем экран, чтобы шли часы
    static unsigned long lastDisplayUpdate = 0;
    if (millis() - lastDisplayUpdate > 1000)
    {
        lastDisplayUpdate = millis();

        // Передаем текущие сохраненные значения имени станции и трека
